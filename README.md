...
The script is actually two different files: `success_metrics.py`  and `reports.py` 
    ...
    `reports.py` is an interactive script that consumes the JSON file generated by `success_metrics.py` and produces different types of reports and graphs depending on the Primary Desired Outcome (PDO). The main output will be a `successmetrics.pdf` report containing graphs and data relevant to the chosen PDO. Additionally, all graphs are also saved to individual .png files for further re-use in presentations.
